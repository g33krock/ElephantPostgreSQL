...
const getUsers = (request, response) => {
  ...
  const createUser = (request, response) => {
    ...
    pool.query('INSERT INTO teachers (teacher_id, firstname, lastname, campus, role) VALUES ($1, $2, $3, $4, $5, $6)', [teacher_id, firstname, lastname, campus, role, image], (error, results) => {
      if (error) {
        throw error
      }
  ...
  const id = parseInt(request.params.id);
  const { firstname, lastname, campus, role, teacher_id, image } = request.body;


  pool.query('UPDATE teachers SET firstname = $1, lastname = $2, campus = $3, role = $4, image = $6 WHERE teacher_id = $5', [firstname, lastname, campus, role, teacher_id, image], (error, results) => {
      if (error) {
        throw error;
      }
